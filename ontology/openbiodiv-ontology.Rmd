...
We incorporate several external ontologies fully into our data model. They are
found under `imported_ontologies`. However, we do not import from the web directly,
as (1) sometimes URL's don't resolve or severs are down; and (2) we removed some
minor bugs (like differing prefix definitions amongst the imported ontologies).

We also borrow parts from external ontologies (contributed parts). Those are
...
##### Class: *Taxonomic Status Vocabulary*

A dictionary of taxonomic statuses.
...
:TaxonomicStatusDictionary rdf:type owl:Class ;
  ...
  rdfs:label "Dictionary of Taxonomic Terms."
  fabio:hasDiscipline <http://dbpedia.org/page/Taxonomy_(biology)> .
```

##### Class: *Taxonomic Status*
...
##### Class: *RCC5 Relationship Dictionary*
...
```
:RCC5RelationshipTerms a :RCC5RelationshipDictionary ;
  skos:prefLabel "The RCC5 Vocabulary Used by OpenBiodiv" .
```
